...
func appendingWrapper(slice *[]int, value int) JobWrapper {
	return func(j Job) Job {
		return FuncJob(func(cCtx *Context) {
			appendingJob(slice, value).Run(cCtx)
			j.Run(cCtx)
		})
	}
}
...
func TestChain(t *testing.T) {
	...
	cCtx := NewContext(context.Background(), 0)
	NewChain(append1, append2, append3).Then(append4).Run(cCtx)
...
func TestChainRecover(t *testing.T) {
	panickingJob := FuncJob(func(cCtx *Context) {
...
func (j *countJob) Run(cCtx *Context) {
	j.m.Lock()
	j.started++
	j.m.Unlock()
	time.Sleep(j.delay)
...
func TestChainSkipIfStillRunning(t *testing.T) {
	...
	t.Run("different jobs independent", func(t *testing.T) {
		...
		for i := 0; i < 11; i++ {
			go wrappedJob1.Run(cCtx)
			go wrappedJob2.Run(cCtx)
		}
