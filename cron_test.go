...
func TestFuncPanicRecovery(t *testing.T) {
	...
	cron.Start()
	defer cron.Stop()
	cron.AddFunc("* * * * * ?", func(cCtx *Context) {
		panic("YOLO")
	})
...
func (d DummyJob) Run(cCtx *Context) {
	panic("YOLO")
}
...
func TestStopCausesJobsToNotRun(t *testing.T) {
	...
	cron.Start()
	cron.Stop()
	cron.AddFunc("* * * * * ?", func(cCtx *Context) { wg.Done() })
...
func TestAddWhileRunningWithDelay(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func(cCtx *Context) { atomic.AddInt64(&calls, 1) })
...
func TestRemoveBeforeRunning(t *testing.T) {
	...
	cron := newWithSeconds()
	id, _ := cron.AddFunc("* * * * * ?", func(cCtx *Context) { wg.Done() })
	cron.Remove(id)
	cron.Start()
	defer cron.Stop()
...
func TestSnapshotEntries(t *testing.T) {
	...
	cron := New()
	cron.AddFunc("@every 2s", func(cCtx *Context) { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestMultipleEntries(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("0 0 0 1 1 ?", func(cCtx *Context) {})
	cron.AddFunc("* * * * * ?", func(cCtx *Context) { wg.Done() })
	id1, _ := cron.AddFunc("* * * * * ?", func(cCtx *Context) { t.Fatal() })
	id2, _ := cron.AddFunc("* * * * * ?", func(cCtx *Context) { t.Fatal() })
	cron.AddFunc("0 0 0 31 12 ?", func(cCtx *Context) {})
	cron.AddFunc("* * * * * ?", func(cCtx *Context) { wg.Done() })
...
func TestRunningMultipleSchedules(t *testing.T) {
	...
	cron.AddFunc("0 0 0 31 12 ?", func(cCtx *Context) {})
	cron.AddFunc("* * * * * ?", func(cCtx *Context) { wg.Done() })
	cron.Schedule(Every(time.Minute), FuncJob(func(cCtx *Context) {}))
	cron.Schedule(Every(time.Second), FuncJob(func(cCtx *Context) { wg.Done() }))
	cron.Schedule(Every(time.Hour), FuncJob(func(cCtx *Context) {}))
...
func TestLocalTimezone(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc(spec, func(cCtx *Context) { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func (t testJob) Run(cCtx *Context) {
	t.wg.Done()
}
...
func TestScheduleAfterRemoval(t *testing.T) {
	...
	cron := newWithSeconds()
	hourJob := cron.Schedule(Every(time.Hour), FuncJob(func(cCtx *Context) {}))
	cron.Schedule(Every(time.Second), FuncJob(func(cCtx *Context) {
		mu.Lock()
		defer mu.Unlock()
		switch calls {
		case 0:
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func(cCtx *Context) { atomic.AddInt64(&calls, 1) })
	cron.Schedule(new(ZeroSchedule), FuncJob(func(cCtx *Context) { t.Error("expected zero task will not run") }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
...
func TestStopAndWait(t *testing.T) {
	...
	t.Run("a couple fast jobs and a slow job added, waits for slow job", func(t *testing.T) {
		...
		cron.AddFunc("* * * * * *", func(cCtx *Context) {})
		cron.Start()
		cron.AddFunc("* * * * * *", func(cCtx *Context) { time.Sleep(2 * time.Second) })
		cron.AddFunc("* * * * * *", func(cCtx *Context) {})
		time.Sleep(time.Second)
